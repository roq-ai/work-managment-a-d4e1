...
type ColumnType = ColumnDef<ProjectInterface, unknown>;
...
export function ProjectListPage(props: ProjectListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getProjects({
        relations: ['company', 'user', 'ticket.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProjectInterface>>(
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('ticket', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Ticket',
          accessorKey: 'ticket',
          cell: ({ row: { original: record } }: any) => record?._count?.ticket,
        }
      : null,
