...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getTickets() {
    ...
    const response = await prisma.ticket
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'ticket'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createTicket() {
    ...
    const body = { ...req.body };
    if (body?.time_entry?.length > 0) {
      const create_time_entry = body.time_entry;
      body.time_entry = {
        create: create_time_entry,
      };
    } else {
      delete body.time_entry;
    }
