...
import { getTimeEntries } from 'apiSdk/time-entries';
...
function DashboardPage() {
  ...
  const teamStat = useSWR<PaginatedInterface<TeamInterface>>(
    () => `/teams`,
    () => getTeams({ limit: 1, offset: 0 }),
  );

  const ticketStat = useSWR<PaginatedInterface<TicketInterface>>(
    () => `/tickets`,
    () => getTickets({ limit: 1, offset: 0 }),
  );

  const timeEntryStat = useSWR<PaginatedInterface<TimeEntryInterface>>(
    () => `/time-entries`,
    () => getTimeEntries({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Teams',
      result: teamStat?.data?.totalCount,
      isLoading: teamStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tickets',
      result: ticketStat?.data?.totalCount,
      isLoading: ticketStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Time Entries',
      result: timeEntryStat?.data?.totalCount,
      isLoading: timeEntryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
