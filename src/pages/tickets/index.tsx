...
type ColumnType = ColumnDef<TicketInterface, unknown>;
...
export function TicketListPage(props: TicketListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTickets({
        relations: ['project', 'user_ticket_creator_idTouser', 'user_ticket_assignee_idTouser', 'time_entry.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['title.contains', 'description.contains', 'status.contains', 'priority.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TicketInterface>>(
    () => `/tickets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_ticket_creator_idTouser',
          header: 'User Ticket Creator Id Touser',
          accessorKey: 'user_ticket_creator_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_ticket_creator_idTouser?.id}`}
            >
              {record.user_ticket_creator_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_ticket_assignee_idTouser',
          header: 'User Ticket Assignee Id Touser',
          accessorKey: 'user_ticket_assignee_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_ticket_assignee_idTouser?.id}`}
            >
              {record.user_ticket_assignee_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('time_entry', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'time_entry',
          header: 'Time Entry',
          accessorKey: 'time_entry',
          cell: ({ row: { original: record } }: any) => record?._count?.time_entry,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/tickets/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
