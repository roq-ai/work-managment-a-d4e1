...
import { getTicketById } from 'apiSdk/tickets';
import { TicketInterface } from 'interfaces/ticket';
import { TimeEntryListPage } from 'pages/time-entries';
...
function TicketViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TicketInterface>(
    ...
    () =>
      getTicketById(id, {
        relations: ['project', 'user_ticket_creator_idTouser', 'user_ticket_assignee_idTouser'],
      }),
  );
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem label="Priority" text={data?.priority} />
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Ticket Creator Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_ticket_creator_idTouser?.id}`}>
                        {data?.user_ticket_creator_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Ticket Assignee Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_ticket_assignee_idTouser?.id}`}>
                        {data?.user_ticket_assignee_idTouser?.email}
                      </Link>
                    }
                  />
                )}
